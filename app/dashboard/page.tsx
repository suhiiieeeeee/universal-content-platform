...
export default async function DashboardOverviewPage() {
  ...
  return <div className="flex flex-1 flex-col">
    <DashboardHeader title={`Welcome back${profile?.display_name ? `, ${profile.display_name}` : ""}`} description="Your files and collections, in one focused workspace." profile={profileForHeader} actions={<Button asChild size="sm"><Link href="/dashboard/files"><Plus data-icon="inline-start" />New file</Link></Button>} />
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      {!collections?.length ? <Empty className="border border-dashed"><EmptyHeader><EmptyMedia variant="icon"><FileText /></EmptyMedia><EmptyTitle>Your files start here</EmptyTitle><EmptyDescription>Create a file directly. Collections are optional and can be added later.</EmptyDescription></EmptyHeader><EmptyContent><Button asChild><Link href="/dashboard/files"><Plus data-icon="inline-start" />Create your first file</Link></Button></EmptyContent></Empty> : <Card><CardHeader><CardTitle className="text-base">Recent collections</CardTitle><CardDescription>Keep your content organized without setup overhead.</CardDescription></CardHeader><CardContent className="flex flex-col gap-2">{collections.map((collection) => <Link key={collection.id} href={`/dashboard/collections/${collection.slug}`} className="group flex items-center justify-between rounded-md border border-border p-3 transition-colors hover:bg-accent"><div className="flex items-center gap-3"><div className="flex size-8 items-center justify-center rounded-md bg-secondary"><Layers className="size-4 text-secondary-foreground" /></div><div className="flex flex-col"><span className="text-sm font-medium">{collection.name}</span><span className="text-xs text-muted-foreground">/{collection.slug}</span></div></div><ArrowUpRight className="size-4 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100" /></Link>)}</CardContent></Card>}
    </div>
  </div>
}
