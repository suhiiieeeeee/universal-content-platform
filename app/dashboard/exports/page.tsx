...
export default async function ExportsPage() {
  ...
  const [{ data: profile }, { data: documents }] = await Promise.all([supabase.from("profiles").select("username, display_name, avatar_url").eq("id", auth.user.id).maybeSingle(), supabase.from("documents").select("name, slug, type, data").eq("user_id", auth.user.id).order("updated_at", { ascending: false })])
