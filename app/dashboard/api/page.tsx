...
export default async function ApiPage() {
  ...
  const [{ data: profile }, { data: collections }, { data: standalone }] = await Promise.all([
    ...
    supabase.from("documents").select("id, slug, name").eq("user_id", auth.user.id).is("collection_id", null).eq("status", "published").eq("visibility", "public"),
  ])
  const username = profile?.username ?? ""
  const endpoints = [...(collections ?? []).map((collection: any) => ({ label: collection.name, path: `/api/v1/u/${username}/${collection.slug}`, count: collection.documents?.length ?? 0 })), ...(standalone ?? []).map((document) => ({ label: document.name ?? document.slug, path: `/api/v1/u/${username}/${document.slug}`, count: 1 }))]
